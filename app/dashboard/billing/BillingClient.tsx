...
export default function BillingClient({ user }: BillingClientProps) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: fetchInvoices is stable
  useEffect(() => {
  ...
  const isTrialExpired =
    isTrial && user.billingPeriodEnd ? new Date() > new Date(user.billingPeriodEnd) : false
  const trialDaysLeft =
    isTrial && user.billingPeriodEnd && !isTrialExpired
      ? Math.max(
          0,
          Math.ceil(
            (new Date(user.billingPeriodEnd).getTime() - Date.now()) / (1000 * 60 * 60 * 24),
          ),
        )
      : 0
  const usagePercent =
    user.usageLimit > 0 ? Math.min((user.usageCount / user.usageLimit) * 100, 100) : 0
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      ...
      {isTrial && (
        ...
        >
          <div
            className={`rounded-xl border-2 p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${isTrialExpired ? 'border-red-200 bg-red-50' : 'border-primary/40 bg-primary/5'}`}
          >
            <div>
              ...
              <>
                ...
                <div className="flex items-center gap-2 text-sm text-dark/70">
                  ...
                  <span>
                    Next billing date:{' '}
                    <span className="text-dark font-medium">
                      {formatDate(user.billingPeriodEnd)}
                    </span>
                  </span>
                </div>
              </>
                    ...
                    <li key={inv.id} className="flex items-center justify-between py-3 gap-4">
                      <div className="text-sm">
                        ...
                        <p className="text-dark/60 mt-0.5 capitalize">
                          {inv.status} · {(inv.amount / 100).toFixed(2)}{' '}
